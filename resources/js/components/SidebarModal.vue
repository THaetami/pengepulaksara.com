<template>
    <div v-if="user" class="xs:hidden">
        <div v-if="show" class="modal-mask">
            <div class="w-full h-screen">
                <div class="flex flex-no-wrap">
                    <div class="w-64 absolute sm:relative bg-indigo-100 shadow h-screen flex-col justify-between">
                        <div class="px-6">
                            <div class="h-16 w-full flex items-center justify-between mt-2 mb-1">
                                <div class="text-2xl text-indigo-800 text-medium" style="font-weight: bold">
                                    Pengepul Aksara
                                </div>
                                <img v-if="user.image" :src="'/storage/upload/' + user.image" class="rounded-full" width="65">
                                <svg v-else xmlns="http://www.w3.org/2000/svg" id="Layer_1" data-name="Layer 1"
                                    class="cls-ahad cls-isnani ml-1 cls-salasatun cls-arbangatun cls-khomsatun cursor-pointer"
                                    width="65" viewBox="0 0 1500 1500">
                                    <logo-comp></logo-comp>
                                </svg>
                            </div>
                            ...
                            <div class="mt-1 flex justify-end hover:text-gray-300 cursor-pointer">
                                <svg @click="$emit('close')" class="w-[1.5rem] h-[1.5rem]" fill="currentColor"
                            ...
                            <ul class="mt-0">
                                <router-link v-if="user" :to="{name: 'ProfilePage'}" @click="$emit('close')" class="flex w-full justify-between text-black cursor-pointer items-center mb-6">
                                    <span class="flex items-center focus:outline-none focus:ring-2 focus:ring-white">
                                        ...
                                        <span class="text-sm ml-2 hover:text-red-500">Update Profile</span>
                                    </span>
                                </router-link>

                                <router-link :to="{ name: 'SastraUser', params: { username: user.username }}" @click="$emit('close')" class="flex w-full justify-between text-black cursor-pointer items-center mb-6">
                                    <span class="flex items-center focus:outline-none focus:ring-2 focus:ring-white">
                                        ...
                                        <span class="text-sm ml-2 hover:text-red-500">My Sastra</span>
                                    </span>
                                </router-link>

                                <li @click.prevent="logout" class="flex w-full justify-between text-black cursor-pointer items-center mb-6">
                                    <span class="flex items-center focus:outline-none focus:ring-2 focus:ring-white">
                                        ...
                                        <span class="text-sm ml-2 hover:text-red-500">Logout</span>
                                    </span>
                                </li>
                            </ul>
                        </div>
