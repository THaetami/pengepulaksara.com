            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="flex-1 min-w-0 p-0">
                    <div class="text-md font-medium flex">
                        <div class="flex justify-center dropdown dropdown-hover">
                            ...
                            <div :tabindex="sastra.user.id" class="bodydropdown justify-center mt-[1.5rem] dropdown-content rounded-md card card-compact w-64 p-0 shadow bg-white">
                                <div class="card-body p-0">
                                    <div class="flex items-center space-x-4">
                                        <div class="flex-1 min-w-0">
                                            <router-link :to="{ name: 'SastraUser', params: { username: sastra.user.username }}" aria-label="page sastra">
                                                <img v-if="sastra.user.image" :src="'/storage/upload/' +sastra.user.image " class="h-12 w-12 rounded-full" :alt="sastra.user.username">
                                                <no-image v-else :sizes='12'></no-image>
                                            </router-link>
                                        </div>
                                        <div class="inline-flex justify-center select-none card-title">
                                            <h1>{{ sastra.user.name  }}</h1>
                                        </div>
                                    </div>

                                    <p class="flex">
                                        <span class="text-sm hover:underline cursor-pointer select-none">{{ sastra.user.username }}</span>
                                        <svg viewBox="0 0 24 24" aria-label="Verified account"
                                            class="h-4 w-4 select-none focus:ring-0 " fill="currentColor" style="color: rgb(29, 155, 240);">
                                            <g>
                                                <path
                                    ...
                                    <div class="flex items-center space-x-4">
                                        <div class="inline-flex justify-center select-none">
                                            <p class="text-sm">Joined: {{ formatDate(sastra.user.created_at)  }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
