<template>
    <div v-for="sastra in sastras" v-bind:key="sastra.id">
        <post-comp :sastra="sastra" @refreshUserPage="hapus"></post-comp>
    </div>
...
export default {
    ...
    methods: {
        ...mapActions(["getuserpage", "gettitleheader", "getsastracount"]),
        async getSastras() {
            try {
                const response = await axios.get(`/api/sastras/${this.$route.params.username}?page=${this.page}`)
                this.sastras = response.data.data.sastras.data
                this.getuserpage({
                    user: response.data.data.user,
                })
                this.getsastracount(response.data.data.count)
        ...
        getNextSastras() {
            ...
            if (bottomOfWindow) {
                this.page++;
                axios.get(`/api/sastras/${this.$route.params.username}?page=${this.page}`)
                .then(response => {
                    this.isLoading = true;
